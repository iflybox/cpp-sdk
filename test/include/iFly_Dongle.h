...
#define IFLY_DONGLE_VERSION(MajorVer, MinorVer)	( ( (unsigned short)MajorVer ) << 16 | ( (unsigned short)MinorVer ) )
	...
	typedef enum
	{
		...
		dm_flex_file        = 12,                   /* FlexNet license file */
	} Dongle_Model;
...
#define DONGLE_PROT_VOICE_Henry     0x00000400  /* voicer 11*/
#define DONGLE_PROT_VOICE_Xiaoqi    0x00000800  /* voicer 12 */
...
#define DONGLE_PROT_VOICE_ALL       0xFFFFFFFF  /* All voices */
	...
	struct IDongle_Prod_Prot_Info
	{
		...
		virtual void prod_ver(int major, int minor, int build) = 0;
		virtual void prod_ver(unsigned int ver) = 0;
		virtual void prod_ver(unsigned int * ver) = 0;
		...
		/* 返回\设置过期期限 */
		virtual time_t expired_time(void) = 0;
		virtual void expired_time(time_t timer) = 0;
	...
	struct IDongle_Prot_Info
	{
		...
		virtual int set_update_pack(const char * buf, const unsigned buf_len) = 0;
	...
	struct IDongle
	{
		...
		/* 获取\设置加密狗实时时钟 */
		virtual int get_real_clock(time_t * timer) = 0;
		virtual int set_real_clock(const time_t timer) = 0;

		/* 获取加密狗的软、硬件信息包(用于远程更新) */
		virtual int get_update_pack(char * buf, int * buf_len) = 0;
		
		virtual int set_update_pack(const char * buf, const unsigned buf_len) = 0;

		virtual int checkout(const char* prod_name, const char* prod_ver) = 0;
		virtual void checkin(const char* prod_name) = 0;

		virtual bool check_dongle(void) = 0;
	};
	...
	struct IDongle_Mngr
	{
		...
		/* 狗管理器初始化 */
		virtual int init(Log_IO *log_io, const char* licence_path, int auto_updata, int disable_cache, void * para = 0) = 0;
	...
	/* 创建加密狗管理接口 */
	IDongle_Mngr * create_dongle_mngr(void);

	/* 销毁加密狗管理接口 */
	void destroy_dongle_mngr(IDongle_Mngr * dongle_mngr);
