...
#ifndef SP_ASSERT
#  if defined(_DEBUG)
#    if defined(WIN32) || defined(ACE_WIN32)
#      include <crtdbg.h>
...
#    else   /* NOT WIN32 */
#      include <assert.h>
#      define SP_ASSERT(expr)				if ( !(expr) ) AssertFailed( #expr, __FILE__, __LINE__, 0 );
#      define SP_VERIFY						SP_ASSERT
#      define SP_ASSERTS(expr, msg)			if ( !(expr) ) { _SP_Assert_ ast; AssertFailed( #expr, __FILE__, __LINE__, ast.assert_msg msg ); }
#      define SP_VERIFYS					SP_ASSERTS
#    endif /* NOT WIN32 */
#  else   /* _DEBUG */
...
#if !defined(WIN32)
/* linux 下增强的ASSERT */
inline void AssertFailed(const char * expr, const char * filename, unsigned int lineno, const char * msg = 0)
{
...
/* 宽字符版本 */
inline void AssertFailed(const wchar_t * expr, const wchar_t * filename, unsigned int lineno, const wchar_t * msg = 0)
{
...
/* 宽字符版本 */
inline void AssertFailed(const wchar_t * expr, const char * filename, unsigned int lineno, const wchar_t * msg = 0)
{
	wchar_t fnw[266]; fnw[0] = 0;
	#ifdef __sun
	mbstowcs_s(fnw, filename, strlen(filename));
	#else
	mbstowcs(fnw, filename, strlen(filename));
	#endif
	AssertFailed(expr, fnw, lineno, msg);
}
#endif	/* !defined(WIN32) */
