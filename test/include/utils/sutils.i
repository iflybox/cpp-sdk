...
int str_to_time(const char * strtm, time_t & tmv)
{
	...
	ts.tm_mday	= get_val_in_range<unsigned short>(day, 1, 31);
				  ...
				  bool ignore_blank /* = false */, bool supp_quote /* = false */, bool trim_quote /* = true */)
{
	...
	for ( int i = 0, cursor = 0; ; i++ )
	{
			...
			)
		{
			subs[0] = 0;
			#if defined(WIN32) && _MSC_VER < 1300 // VC++ 6.0
			int len = min((i - cursor), SP_COUNTOF(subs) - 1);
			#else
			int len = std::min<int>(i - cursor, SP_COUNTOF(subs) - 1);
			#endif
			if ( len > 0 )
			{
				if ( !supp_quote )
...
{
	str_arr arr;
	split_str(str, arr, splt, trim, true, supp_quote);
...
template<class LIST>
SP_INLINE
int get_file_list(const char* dir, LIST & flist, const char* filter /* = 0 */, bool recursive /* = true */, int max_file/* = -1*/)
{
	return get_file_list(dir, flist, filter, recursive, ff_proc_default(), max_file);
}

template<class LIST, class FF_PROC>
SP_INLINE
int get_file_list(const char* dir, LIST & flist, const char* filter /* = 0 */, bool recursive /* = true */, FF_PROC fproc /* = 0 */, int max_file/* = -1*/)
{
	int ret = 0;
	char flt[MAX_PATH] = "*.*";
	...
	do
	{
		...
		if ( find_data.attrib & _A_SUBDIR )
		{
			if ( recursive )
			{
				...
				ret = get_file_list(find_path, flist, filter, recursive, fproc, max_file);
				if ( ret != 0 )
					break;
			}
			continue;
		...
		if ( fproc(find_file) ) {
			if (max_file != -1 && (int)flist.size() >= max_file)
				break;
	...
	for ( ; ; )
	{
		...
		if ( st.st_mode & S_IFDIR )
		{
			if ( recursive )
			{
				ret = get_file_list(find_file, flist, filter, recursive, fproc, max_file);
				if ( ret != 0 )
					break;
			}
			continue ;
