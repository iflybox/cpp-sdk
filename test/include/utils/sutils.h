...
#define SP_STRLEN(arr)				( SP_COUNTOF(arr) - 1 )
...
namespace sp
{
	...
	*/
	...
	/** 
	 * @fn		* trans_escape_char
	 * @brief	把类似于“\r\n”的字符替换成实际的字符
	 * @author	jdyu
	 * @date	2010/1/12
	 * @return	const char * 返回目标缓冲区
	 * @param	std::string & str	- [in/out] 传入传出
	 * @see		
	 */
...
namespace sp
{
	...
	*/
	...
	int get_file_list(const char* dir, LIST & flist, const char* filter = 0, bool recursive = true, FF_PROC fproc = ff_proc_default(), int max_file = -1);
	template<class LIST>
...
namespace sp
{
	...
	std::string get_cfg(const char * cfg_default = 0);
	/** 
	 * @brief 	install_path
	 *
	 * 	获得/设置当前系统的安装路径
